...
def cart_add(request):
    ...
    product_id = int(request.POST.get('product_id'))               #參數轉為 int，作為輸入驗證 進入業務邏輯前改正確型別
    ...
    current_cart_qty = cart.cart.get(str(product.id), {}).get("qty", 0)  #購物車裡找這個商品，如果有，就拿它的數量；沒有數量就拿 0。

    # 已有數量+新增量 > 庫存 → 禁止
    if current_cart_qty + product_quantity > product.available_stock:
        return JsonResponse({'error': '超過庫存數量'}, status=400)

    cart.add(product, product_quantity)   # 更新使用者的購物車資料到 session
