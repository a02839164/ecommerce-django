...
開發目標是 **「打造可實際營運的生產級後端系統」**。我專注於解決真實商業情境下的核心痛點：
...
系統以 Django 為核心，採多 App 架構劃分業務職責。所有複雜的跨模組流程，皆由明確定義的 **Service Layer** 協調。

此設計旨在避免將業務邏輯分散於 Views 或 Models 中，從而提高程式碼的可讀性、可測試性與專案擴大後的可維護性。

---

## 🛠 核心設計決策與工程亮點 (Key Design Decisions & Engineering Highlights)

這部分著重於解決實際挑戰的策略，這是在面試中展示技術深度最重要的環節。
...
我們不依賴前端或即時 API 回傳的結果，而是將 **PayPal Webhook** 視為最終可信來源。
...
針對電商常見的濫用風險，我在關鍵節點加入了防護機制：
* **機器人防禦：** 整合 Cloudflare Turnstile 於登入與註冊介面。
* **帳號驗證：** 強制 Email 驗證流程。
